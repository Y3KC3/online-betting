...
export function CardMyBets({ game, bets, idEvent, idUser, amount, setEventBet }) {
    const navigate = useNavigate();
    return (
        <div className="p-2 my-2" style={{ border: '1px solid #fff' }}>
            <h1 style={{ fontSize: '18px' }}>{game}</h1>
            <h1 style={{ fontSize: '18px' }}>{bets}</h1>
            <button className="btn btn-sm text-light" style={{ background: '#fb7b33' }} onClick={() => { cancelBet(idEvent, idUser, amount, setEventBet,navigate); }}>Cancelar</button>
        </div>
    );
};
